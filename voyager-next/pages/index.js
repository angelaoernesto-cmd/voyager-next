...
async function callAI(prompt) {
  // ✅ FIX #6: URL dinámica — usa la URL de Vercel en producción/APK,
  // y ruta relativa /api/ai en desarrollo local.
  // NUNCA hardcodear una URL fija: si cambias el proyecto en Vercel,
  // la URL hardcodeada deja de funcionar silenciosamente.
  const baseUrl =
    typeof window !== "undefined" && window.location.hostname !== "localhost"
      ? "https://voyager-next-chi.vercel.app" // ← Tu dominio de Vercel
      : "";                                    // ← Ruta relativa en local
...
"order" = orden lógico de visita, "days" = días recomendados.`);
      ...
      // ✅ FIX #7: Muestra el error real para facilitar el diagnóstico.
      console.error("Error crítico leyendo JSON de IA:", e);
      sError(`Error IA: ${e.message || "Sin conexión. Verifica Vercel y GEMINI_API_KEY."}`);
    }
    sLoading(false);
  },[dest,month,year]);
